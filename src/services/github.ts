      ...
      try {
        ...
        for await (const response of this.octokit.paginate.iterator(
          ...
          if (pageCount >= MAX_SEARCH_PAGES) {
            logger.debug(`Reached max search pages (${MAX_SEARCH_PAGES}), stopping pagination.`);
            return items;
          }
        }
        return items;
        ...
        if (isRateLimit) {
          // Graceful degradation: return already-fetched data if available
          if (items.length > 0) {
            logger.warn(`Rate limited during pagination (attempt ${attempt}/${MAX_PAGINATION_RETRIES}). Returning ${items.length} items fetched so far.`);
            return items;
          }
